// Crash calculator v3 — host-callback protocol.
// Multiplayer crash game with rising multiplier and random crash point.
//
// Flow (v3):
//   place_bet (per player) → reserve, add to entry list in KV.
//     First player: schedule_wakeup at height+joinWindow (timeout).
//   bet_action(cashout) → queue cashout in KV for next tick.
//   block_update → timeout: OPEN→RESOLVING, start tick loop.
//     tick: get_rng → crash check → settle cashouts → schedule next tick.
//
// Session state lives in WASM KV. Multiple place_bet calls share the same KV.
// The first player's betID acts as the session trigger for wakeups.
//
// Exports: alloc, dealloc, place_bet, bet_action, block_update, info
package main

import (
	"encoding/binary"
	"math"
	"unsafe"
)

// ---------------------------------------------------------------------------
// Host imports
// ---------------------------------------------------------------------------

//go:wasmimport env kv_get
func kv_get(keyPtr, keyLen uint32) uint64

//go:wasmimport env kv_set
func kv_set(keyPtr, keyLen, valPtr, valLen uint32)

//go:wasmimport env schedule_wakeup
func schedule_wakeup(betID, height uint64)

//go:wasmimport env reserve
func host_reserve(betID, amount uint64) uint32

//go:wasmimport env settle
func host_settle(betID, payout uint64, kind uint32) uint32

//go:wasmimport env get_rng
func host_get_rng(height uint64, outPtr uint32) uint32

//go:wasmimport env get_bet_count
func host_get_bet_count() uint32

//go:wasmimport env get_bet_id
func host_get_bet_id(index uint32) uint64

//go:wasmimport env emit_event
func host_emit_event(topicPtr, topicLen, dataPtr, dataLen uint32)

// ---------------------------------------------------------------------------
// Memory
// ---------------------------------------------------------------------------

//export alloc
func alloc(size uint32) *byte {
	if size == 0 {
		size = 1
	}
	buf := make([]byte, size)
	return &buf[0]
}

//export dealloc
func dealloc(ptr *byte, size uint32) {}

// ---------------------------------------------------------------------------
// Constants & layouts
// ---------------------------------------------------------------------------

const (
	kindWin  uint32 = 1
	kindLoss uint32 = 2

	phaseOpen      byte = 0
	phaseResolving byte = 1
	phaseSettled   byte = 2

	statusActive     byte = 0
	statusCashoutReq byte = 1
	statusSettled    byte = 2

	cfgSize   = 40
	roundSize = 33
	entrySize = 18
)

// Config (40 bytes): house_edge(8)+tick_growth(8)+max_mult(8)+max_ticks(8)+join_window(8)
// Round (33 bytes): current_mult(8)+ticks_elapsed(8)+pending_height(8)+phase(1)+entry_count(8)
// Entry (18 bytes): entry_id(8)+stake(8)+status(1)+pad(1)

var (
	keyCfg       = []byte("cfg")
	keyRound     = []byte("r")
	keyEntryList = []byte("el")
	keyTrigger   = []byte("tg") // stores the trigger betID for wakeups
)

func entryKey(id uint64) []byte {
	buf := make([]byte, 9)
	buf[0] = 'e'
	binary.LittleEndian.PutUint64(buf[1:], id)
	return buf
}

func cashoutKey(height uint64) []byte {
	buf := make([]byte, 9)
	buf[0] = 'c'
	binary.LittleEndian.PutUint64(buf[1:], height)
	return buf
}

// ---------------------------------------------------------------------------
// place_bet — player joins the round
// ---------------------------------------------------------------------------

// ---------------------------------------------------------------------------
// init_game — called once on BankrollAddCalculator. Starts the round loop.
// ---------------------------------------------------------------------------

//export init_game
func init_game(sentinelID, bankrollID, calculatorID uint64) {
	// Store sentinel as the permanent trigger for wakeups.
	buf := make([]byte, 8)
	binary.LittleEndian.PutUint64(buf, sentinelID)
	kvSet(keyTrigger, buf)

	// Create first round.
	round := newRound()
	kvSet(keyRound, round)

	// Start round number tracking.
	rnBuf := make([]byte, 8)
	binary.LittleEndian.PutUint64(rnBuf, 1)
	kvSet([]byte("rn"), rnBuf)

	// Schedule first wakeup.
	schedule_wakeup(sentinelID, 0)

	emitJSON("init", "sentinel_id", sentinelID, "round", uint64(1))
}

// ---------------------------------------------------------------------------
// place_bet — player joins the round
// ---------------------------------------------------------------------------

//export place_bet
func place_bet(betID, bankrollID, calculatorID, stake uint64, paramsPtr, paramsLen uint32) uint32 {
	cfg := kvGetOrInitCfg()

	round := kvGetRound()
	if round == nil {
		// Very first round ever — create it and schedule the first wakeup.
		round = newRound()
		kvDelete(keyEntryList)
	}
	// Accept bets during OPEN phase only.
	if round[24] != phaseOpen {
		return 10 // not accepting bets
	}

	// Prevent duplicate.
	if kvGetBytes(entryKey(betID)) != nil {
		return 11
	}

	// Store entry.
	entry := make([]byte, entrySize)
	binary.LittleEndian.PutUint64(entry[0:], betID)
	binary.LittleEndian.PutUint64(entry[8:], stake)
	entry[16] = statusActive
	kvSet(entryKey(betID), entry)
	appendEntryID(betID)

	// Update count.
	count := binary.LittleEndian.Uint64(round[25:33])
	count++
	binary.LittleEndian.PutUint64(round[25:], count)
	kvSet(keyRound, round)

	// Reserve max payout.
	maxMult := binary.LittleEndian.Uint64(cfg[16:24])
	maxPayout := safeMulDiv(stake, maxMult, 10000)
	if host_reserve(betID, maxPayout) != 0 {
		return 3
	}

	emitJSON("joined", "entry_id", betID, "stake", stake, "players", count)
	return 0
}

// ---------------------------------------------------------------------------
// bet_action — cashout request
// ---------------------------------------------------------------------------

//export bet_action
func bet_action(betID uint64, actionPtr, actionLen uint32) uint32 {
	round := kvGetRound()
	if round == nil || round[24] != phaseResolving {
		return 20 // not resolving
	}

	entry := kvGetBytes(entryKey(betID))
	if entry == nil || entry[16] != statusActive {
		return 21 // not active
	}

	// Mark cashout requested.
	entry[16] = statusCashoutReq
	kvSet(entryKey(betID), entry)

	// Queue at current pending_height (the tick being resolved).
	pendingH := binary.LittleEndian.Uint64(round[16:24])
	appendCashout(pendingH, betID)

	return 0
}

// ---------------------------------------------------------------------------
// block_update — timeout + tick loop
// ---------------------------------------------------------------------------

//export block_update
func block_update(height uint64) {
	count := host_get_bet_count()
	if count == 0 {
		return
	}

	round := kvGetRound()
	if round == nil {
		return
	}

	switch round[24] {
	case phaseOpen:
		handleTimeout(height, round)
	case phaseResolving:
		handleTick(height, round)
	case phaseSettled:
		// Nothing to do — next place_bet will create new round.
	}
}

func handleTimeout(height uint64, round []byte) {
	cfg := kvGetOrInitCfg()
	joinWindow := binary.LittleEndian.Uint64(cfg[32:40])

	// Check if join window has elapsed.
	// We don't store the start height, so use a simple approach:
	// The first wakeup fires at the block after place_bet (height=0 means next block).
	// We re-schedule joinWindow-1 more times to wait.
	// Use a counter in KV to track remaining wait blocks.
	waitKey := []byte("wt")
	waitBuf := kvGetBytes(waitKey)
	var remaining uint64
	if waitBuf == nil {
		remaining = joinWindow
	} else {
		remaining = binary.LittleEndian.Uint64(waitBuf)
	}

	if remaining > 1 {
		remaining--
		buf := make([]byte, 8)
		binary.LittleEndian.PutUint64(buf, remaining)
		kvSet(waitKey, buf)
		// Re-schedule trigger.
		trigBuf := kvGetBytes(keyTrigger)
		if trigBuf != nil {
			trigID := binary.LittleEndian.Uint64(trigBuf)
			schedule_wakeup(trigID, height+1)
		}
		return
	}

	// Join window elapsed — transition to RESOLVING.
	kvDelete(waitKey)
	round[24] = phaseResolving
	binary.LittleEndian.PutUint64(round[16:], height)
	kvSet(keyRound, round)

	// Schedule tick for next block using trigger betID.
	trigBuf := kvGetBytes(keyTrigger)
	if trigBuf != nil {
		trigID := binary.LittleEndian.Uint64(trigBuf)
		schedule_wakeup(trigID, height+1)
	}

	count := binary.LittleEndian.Uint64(round[25:33])
	emitJSON("started", "players", count)
}

func handleTick(height uint64, round []byte) {
	cfg := kvGetOrInitCfg()

	pendingHeight := binary.LittleEndian.Uint64(round[16:24])
	currentMult := binary.LittleEndian.Uint64(round[0:8])
	ticksElapsed := binary.LittleEndian.Uint64(round[8:16])
	houseEdge := binary.LittleEndian.Uint64(cfg[0:8])
	tickGrowth := binary.LittleEndian.Uint64(cfg[8:16])
	maxMult := binary.LittleEndian.Uint64(cfg[16:24])
	maxTicks := binary.LittleEndian.Uint64(cfg[24:32])

	// Get RNG for pending height.
	rngBuf := make([]byte, 32)
	ok := host_get_rng(pendingHeight, uint32(uintptr(unsafe.Pointer(&rngBuf[0]))))
	if ok == 0 {
		// RNG not available — reschedule.
		trigBuf := kvGetBytes(keyTrigger)
		if trigBuf != nil {
			trigID := binary.LittleEndian.Uint64(trigBuf)
			schedule_wakeup(trigID, height+1)
		}
		return
	}

	// Compute next multiplier.
	nextMult := currentMult * (10000 + tickGrowth) / 10000
	if nextMult > maxMult {
		nextMult = maxMult
	}
	if nextMult <= currentMult {
		nextMult = currentMult + 1
	}

	// Survival probability.
	var probSurvive float64
	if ticksElapsed == 0 {
		edge := float64(houseEdge) / 10000.0
		probSurvive = (1.0 - edge) * (float64(currentMult) / float64(nextMult))
	} else {
		probSurvive = float64(currentMult) / float64(nextMult)
	}
	if probSurvive > 1.0 {
		probSurvive = 1.0
	}

	// RNG check.
	rngVal := getUniformProb(rngBuf, pendingHeight)
	crashed := rngVal >= probSurvive

	// Load cashouts for this tick.
	cashoutIDs := loadAndDeleteCashoutIDs(pendingHeight)

	payoutMult := nextMult

	if crashed {
		// All remaining entries lose (including cashouts at this tick).
		settleAllAsLoss(round, height)
		emitJSON("crash", "crash_mult_bp", currentMult, "tick", ticksElapsed)
		return
	}

	// Survived — settle cashouts as wins.
	for _, eid := range cashoutIDs {
		entry := kvGetBytes(entryKey(eid))
		if entry == nil || entry[16] == statusSettled {
			continue
		}
		stake := binary.LittleEndian.Uint64(entry[8:16])
		payout := safeMulDiv(stake, payoutMult, 10000)
		entry[16] = statusSettled
		kvSet(entryKey(eid), entry)
		host_settle(eid, payout, kindWin)
		emitJSON("cashout", "entry_id", eid, "mult_bp", payoutMult, "payout", payout)
	}

	// Check max_ticks.
	if maxTicks > 0 && ticksElapsed+1 >= maxTicks {
		settleRemainingAsWin(round, nextMult, height)
		emitJSON("crash", "crash_mult_bp", nextMult, "tick", ticksElapsed+1, "reason", "max_ticks")
		return
	}

	// Check max multiplier cap.
	if nextMult >= maxMult {
		settleRemainingAsWin(round, maxMult, height)
		emitJSON("crash", "crash_mult_bp", maxMult, "tick", ticksElapsed, "reason", "max_mult")
		return
	}

	// Advance to next tick.
	binary.LittleEndian.PutUint64(round[0:], nextMult)
	binary.LittleEndian.PutUint64(round[8:], ticksElapsed+1)
	binary.LittleEndian.PutUint64(round[16:], height) // next tick uses this height's RNG
	kvSet(keyRound, round)

	emitJSON("tick", "mult_bp", nextMult, "tick", ticksElapsed+1)

	// Schedule next tick.
	trigBuf := kvGetBytes(keyTrigger)
	if trigBuf != nil {
		trigID := binary.LittleEndian.Uint64(trigBuf)
		schedule_wakeup(trigID, height+1)
	}
}

// ---------------------------------------------------------------------------
// Settlement helpers
// ---------------------------------------------------------------------------

func settleAllAsLoss(round []byte, height uint64) {
	entryIDs := loadEntryIDs()
	for _, eid := range entryIDs {
		entry := kvGetBytes(entryKey(eid))
		if entry == nil || entry[16] == statusSettled {
			continue
		}
		entry[16] = statusSettled
		kvSet(entryKey(eid), entry)
		host_settle(eid, 0, kindLoss)
	}
	markSettledAndRestart(round, height)
}

func settleRemainingAsWin(round []byte, multBP, height uint64) {
	entryIDs := loadEntryIDs()
	for _, eid := range entryIDs {
		entry := kvGetBytes(entryKey(eid))
		if entry == nil || entry[16] == statusSettled {
			continue
		}
		stake := binary.LittleEndian.Uint64(entry[8:16])
		payout := safeMulDiv(stake, multBP, 10000)
		entry[16] = statusSettled
		kvSet(entryKey(eid), entry)
		host_settle(eid, payout, kindWin)
	}
	markSettledAndRestart(round, height)
}

func markSettledAndRestart(round []byte, height uint64) {
	entryIDs := loadEntryIDs()
	for _, eid := range entryIDs {
		kvDelete(entryKey(eid))
	}
	kvDelete(keyEntryList)
	kvDelete([]byte("wt"))

	// Auto-restart: new round immediately.
	newR := newRound()
	kvSet(keyRound, newR)

	// Re-schedule trigger for next block (join window starts).
	trigBuf := kvGetBytes(keyTrigger)
	if trigBuf != nil {
		trigID := binary.LittleEndian.Uint64(trigBuf)
		schedule_wakeup(trigID, height+1)
	}

	// Increment round number.
	rnBuf := kvGetBytes([]byte("rn"))
	rn := uint64(1)
	if rnBuf != nil && len(rnBuf) >= 8 {
		rn = binary.LittleEndian.Uint64(rnBuf) + 1
	}
	buf := make([]byte, 8)
	binary.LittleEndian.PutUint64(buf, rn)
	kvSet([]byte("rn"), buf)

	emitJSON("round_open", "round", rn)
}

func newRound() []byte {
	round := make([]byte, roundSize)
	binary.LittleEndian.PutUint64(round[0:], 10000) // 1.00x
	round[24] = phaseOpen
	return round
}

// ---------------------------------------------------------------------------
// Math
// ---------------------------------------------------------------------------

func safeMulDiv(a, b, c uint64) uint64 {
	if c == 0 {
		return 0
	}
	if a <= 0xFFFFFFFF || b <= 0xFFFFFFFF {
		return a * b / c
	}
	aH, aL := a>>32, a&0xFFFFFFFF
	bH, bL := b>>32, b&0xFFFFFFFF
	mid1 := aH * bL
	mid2 := aL * bH
	low := aL * bL
	high := aH * bH
	carry := uint64(0)
	midSum := mid1 + mid2
	if midSum < mid1 {
		carry = 1
	}
	high += carry
	high += midSum >> 32
	low += midSum << 32
	if low < midSum<<32 {
		high++
	}
	if high == 0 {
		return low / c
	}
	q := high / c
	r := high % c
	return q<<64 + (r<<64+low)/c
}

func getUniformProb(seed []byte, tickH uint64) float64 {
	var buf [8]byte
	binary.LittleEndian.PutUint64(buf[:], tickH)
	data := make([]byte, len(seed)+8)
	copy(data, seed)
	copy(data[len(seed):], buf[:])
	sum := sha256sum(data)
	val := binary.BigEndian.Uint64(sum[0:8])
	den := float64(^uint64(0))
	x := float64(val) / den
	if x <= 0 {
		return math.SmallestNonzeroFloat64
	}
	return x
}

// ---------------------------------------------------------------------------
// KV helpers
// ---------------------------------------------------------------------------

func kvGetBytes(key []byte) []byte {
	if len(key) == 0 {
		return nil
	}
	packed := kv_get(uint32(uintptr(unsafe.Pointer(&key[0]))), uint32(len(key)))
	if packed == 0 {
		return nil
	}
	ptr := uint32(packed >> 32)
	length := uint32(packed & 0xFFFFFFFF)
	if length == 0 {
		return nil
	}
	val := unsafe.Slice((*byte)(unsafe.Pointer(uintptr(ptr))), length)
	if length == 1 && val[0] == 0xFF {
		return nil // delete sentinel
	}
	return val
}

func kvSet(key, value []byte) {
	if len(key) == 0 || len(value) == 0 {
		return
	}
	kv_set(
		uint32(uintptr(unsafe.Pointer(&key[0]))), uint32(len(key)),
		uint32(uintptr(unsafe.Pointer(&value[0]))), uint32(len(value)),
	)
}

var kvDeleteSentinel = []byte{0xFF}

func kvDelete(key []byte) {
	kv_set(
		uint32(uintptr(unsafe.Pointer(&key[0]))), uint32(len(key)),
		uint32(uintptr(unsafe.Pointer(&kvDeleteSentinel[0]))), 1,
	)
}

func kvGetRound() []byte {
	v := kvGetBytes(keyRound)
	if v != nil && len(v) >= roundSize {
		return v
	}
	return nil
}

func kvGetOrInitCfg() []byte {
	cfg := kvGetBytes(keyCfg)
	if cfg != nil && len(cfg) >= cfgSize {
		return cfg
	}
	cfg = make([]byte, cfgSize)
	binary.LittleEndian.PutUint64(cfg[0:], 200)       // house_edge_bp
	binary.LittleEndian.PutUint64(cfg[8:], 350)        // tick_growth_bp
	binary.LittleEndian.PutUint64(cfg[16:], 1_000_000) // max_multiplier_bp = 100x
	binary.LittleEndian.PutUint64(cfg[24:], 0)         // max_ticks = unlimited
	binary.LittleEndian.PutUint64(cfg[32:], 3)         // join_window_blocks
	kvSet(keyCfg, cfg)
	return cfg
}

func appendEntryID(id uint64) {
	list := kvGetBytes(keyEntryList)
	buf := make([]byte, 8)
	binary.LittleEndian.PutUint64(buf, id)
	if list == nil {
		kvSet(keyEntryList, buf)
	} else {
		newList := make([]byte, len(list)+8)
		copy(newList, list)
		copy(newList[len(list):], buf)
		kvSet(keyEntryList, newList)
	}
}

func loadEntryIDs() []uint64 {
	list := kvGetBytes(keyEntryList)
	if list == nil {
		return nil
	}
	n := len(list) / 8
	ids := make([]uint64, n)
	for i := 0; i < n; i++ {
		ids[i] = binary.LittleEndian.Uint64(list[i*8:])
	}
	return ids
}

func appendCashout(height, entryID uint64) {
	key := cashoutKey(height)
	list := kvGetBytes(key)
	buf := make([]byte, 8)
	binary.LittleEndian.PutUint64(buf, entryID)
	if list == nil {
		kvSet(key, buf)
	} else {
		newList := make([]byte, len(list)+8)
		copy(newList, list)
		copy(newList[len(list):], buf)
		kvSet(key, newList)
	}
}

func loadAndDeleteCashoutIDs(height uint64) []uint64 {
	key := cashoutKey(height)
	list := kvGetBytes(key)
	if list == nil {
		return nil
	}
	kvDelete(key)
	n := len(list) / 8
	ids := make([]uint64, n)
	for i := 0; i < n; i++ {
		ids[i] = binary.LittleEndian.Uint64(list[i*8:])
	}
	return ids
}

// ---------------------------------------------------------------------------
// Event helpers
// ---------------------------------------------------------------------------

func emitJSON(topic string, pairs ...interface{}) {
	json := fmtJSON(pairs...)
	topicBytes := []byte(topic)
	jsonBytes := []byte(json)
	host_emit_event(
		uint32(uintptr(unsafe.Pointer(&topicBytes[0]))), uint32(len(topicBytes)),
		uint32(uintptr(unsafe.Pointer(&jsonBytes[0]))), uint32(len(jsonBytes)),
	)
}

func fmtJSON(pairs ...interface{}) string {
	buf := make([]byte, 0, 128)
	buf = append(buf, '{')
	for i := 0; i < len(pairs)-1; i += 2 {
		if i > 0 {
			buf = append(buf, ',')
		}
		key := pairs[i].(string)
		buf = append(buf, '"')
		buf = append(buf, key...)
		buf = append(buf, '"', ':')
		switch v := pairs[i+1].(type) {
		case uint64:
			buf = appendUint(buf, v)
		case string:
			buf = append(buf, '"')
			buf = append(buf, v...)
			buf = append(buf, '"')
		}
	}
	buf = append(buf, '}')
	return string(buf)
}

func appendUint(buf []byte, v uint64) []byte {
	if v == 0 {
		return append(buf, '0')
	}
	var tmp [20]byte
	i := len(tmp)
	for v > 0 {
		i--
		tmp[i] = byte('0' + v%10)
		v /= 10
	}
	return append(buf, tmp[i:]...)
}

// ---------------------------------------------------------------------------
// SHA-256 (inline)
// ---------------------------------------------------------------------------

var sha256K = [64]uint32{
	0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
	0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
	0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
	0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
	0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
	0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
	0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
	0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
}

func sha256sum(data []byte) [32]byte {
	h0 := uint32(0x6a09e667)
	h1 := uint32(0xbb67ae85)
	h2 := uint32(0x3c6ef372)
	h3 := uint32(0xa54ff53a)
	h4 := uint32(0x510e527f)
	h5 := uint32(0x9b05688c)
	h6 := uint32(0x1f83d9ab)
	h7 := uint32(0x5be0cd19)
	msgLen := len(data)
	bitLen := uint64(msgLen) * 8
	data = append(data, 0x80)
	for len(data)%64 != 56 {
		data = append(data, 0)
	}
	var lenBuf [8]byte
	binary.BigEndian.PutUint64(lenBuf[:], bitLen)
	data = append(data, lenBuf[:]...)
	var w [64]uint32
	for off := 0; off < len(data); off += 64 {
		block := data[off : off+64]
		for i := 0; i < 16; i++ {
			w[i] = binary.BigEndian.Uint32(block[i*4:])
		}
		for i := 16; i < 64; i++ {
			s0 := rotr32(w[i-15], 7) ^ rotr32(w[i-15], 18) ^ (w[i-15] >> 3)
			s1 := rotr32(w[i-2], 17) ^ rotr32(w[i-2], 19) ^ (w[i-2] >> 10)
			w[i] = w[i-16] + s0 + w[i-7] + s1
		}
		a, b, c, d, e, f, g, h := h0, h1, h2, h3, h4, h5, h6, h7
		for i := 0; i < 64; i++ {
			S1 := rotr32(e, 6) ^ rotr32(e, 11) ^ rotr32(e, 25)
			ch := (e & f) ^ (^e & g)
			temp1 := h + S1 + ch + sha256K[i] + w[i]
			S0 := rotr32(a, 2) ^ rotr32(a, 13) ^ rotr32(a, 22)
			maj := (a & b) ^ (a & c) ^ (b & c)
			temp2 := S0 + maj
			h = g; g = f; f = e; e = d + temp1; d = c; c = b; b = a; a = temp1 + temp2
		}
		h0 += a; h1 += b; h2 += c; h3 += d; h4 += e; h5 += f; h6 += g; h7 += h
	}
	var out [32]byte
	binary.BigEndian.PutUint32(out[0:], h0)
	binary.BigEndian.PutUint32(out[4:], h1)
	binary.BigEndian.PutUint32(out[8:], h2)
	binary.BigEndian.PutUint32(out[12:], h3)
	binary.BigEndian.PutUint32(out[16:], h4)
	binary.BigEndian.PutUint32(out[20:], h5)
	binary.BigEndian.PutUint32(out[24:], h6)
	binary.BigEndian.PutUint32(out[28:], h7)
	return out
}

func rotr32(x uint32, n uint) uint32 { return (x >> n) | (x << (32 - n)) }

// ---------------------------------------------------------------------------
// info
// ---------------------------------------------------------------------------

//export info
func info() *byte {
	data := []byte(`{"name":"Crash","engine":"crash","mode":"v3","house_edge_bp":200,"developer":"ExoHash","description":"Multiplayer crash — rising multiplier with random crash point"}`)
	result := make([]byte, 4+len(data))
	binary.LittleEndian.PutUint32(result[0:4], uint32(len(data)))
	copy(result[4:], data)
	return &result[0]
}

func main() {}
